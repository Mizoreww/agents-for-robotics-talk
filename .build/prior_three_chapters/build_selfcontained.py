...
order=[1,2,3,4,'exec',5,6,7,8,9,'world',10,11,12,13,'improve',14,15,16,17,18,19,20,22]
assert len(order)==24
minutes=[.5,2.5,2.5,1,2,2.5,3,3,2.5,1.5,2,3,4,3.5,2.5,2,3,4.5,3.5,3.5,2.5,1.5,2,1.5]
...
report={'slides':24,'videos':len(new_media),'media_seconds':sum(m['duration_seconds'] for m in new_media),'minutes':sum(minutes),'chapter_opening_slides':[n for n,key in enumerate(order,1) if not isinstance(key,int)],'bytes':dest.stat().st_size,'sha256':hashlib.sha256(dest.read_bytes()).hexdigest(),'standalone_file':str(dest),'media':[{k:m[k] for k in ['name','slide','sha256','duration_seconds']} for m in new_media],'images':image_audit,'native_text':True,'all_fonts_embedded':True}
