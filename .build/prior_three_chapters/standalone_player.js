...
for(const s of deck.slides){
 ...
 const o=document.createElement('option');o.value=s.number;o.textContent=String(s.number).padStart(2,'0')+' / 24 · '+s.title;g.append(o);
}
...
function notes(s){
 if(allScript){
  ...
  $('timing').textContent='中文讲稿 · 专有名词保留英文 · 24 slides · 约 60 分钟，含视频、读图与停顿';
...
async function show(number){
 ...
 $('message').textContent=clips.length?'Preparing embedded video'+(clips.length>1?'s':'')+'…':'Slide '+current+' / 24 · '+s.group;
...
document.addEventListener('keydown',e=>{
 ...
 else if(e.key==='Home'){e.preventDefault();show(1)}else if(e.key==='End'){e.preventDefault();show(24)}
