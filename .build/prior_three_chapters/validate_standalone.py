...
deck=json.loads(p.scripts['deck-data']);assert len(deck['slides'])==24 and len(deck['media'])==15
assert [r['number'] for r in deck['slides']]==list(range(1,25))
expected_order=[1,2,3,4,None,5,6,7,8,9,None,10,11,12,13,None,14,15,16,17,18,19,20,22]
assert [r['old_slide'] for r in deck['slides']]==expected_order,'Chapter diagrams must open their chapters.'
opening_slides=[r['number'] for r in deck['slides'] if r['old_slide'] is None]
assert opening_slides==[5,11,16]
for number in opening_slides:
 ...
 assert 'Chapter overview' in chapter['html'] or 'Human-assisted setup' in chapter['html']
...
assert len(layout['slides'])==24 and all(not r['overflow'] and r['images'] and all(i['loaded'] for i in r['images']) for r in layout['slides'])
assert [r['number'] for r in layout['slides']]==list(range(1,25))
