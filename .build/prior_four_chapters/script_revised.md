...
第一章从 Agent 直接输出 action 开始：通用模型能不能自己决定机器人下一步怎么动？接着用 Claude Plays Robotics 比较不同 interface 带来的帮助，观察语义目标、任务拆解与实际执行之间的差距。最后用 RPent 收束：怎样把直接控制和各种 action tools 组织成一个可组合的系统？
...
这一章按问题一步步走：先看直接动作能做到哪，再看 interface 怎样补上缺少的能力，最后用 RPent 说明这些不同能力怎样放到一起。

## 06. Direct Actions and Controller Code
...
## 07. Claude Plays Robotics: Model and Interface
...
## 08. VLA Tools: Grounding and Supervision
...
这也说明，模型在目标理解和粗粒度操作上的进步，不会自动转化为同幅度的 fine manipulation 提升。最后几毫米取决于接触状态、连续反馈和动作修正。下面用一个持续接触的 demo，把 Agent + Tools 的优势和当前缺口放在一起总结。

## 10. Agent + Tools: Strengths and Gaps

> 播放与指图提示：播放约 47 秒的 wiping 视频，同时讲右侧优势与缺口。视频是应用示例，不是 force control 或精细操作能力的验证。

到这里，可以把 Agent + Tools 的能力结构讲清楚。它适合把自然语言目标转成可执行的任务安排，理解物体和操作之间的语义关系，拆分 subgoals，再组合现有工具。在一些底层 policy 不熟悉的目标上，这种重新组织指令和技能的能力可以帮助任务迁移。

这也是我们把 Agent 放在长程任务上层的理由：让它负责目标、顺序、进度检查和异常处理，让 action tool 完成局部技能。但长程任务拆解有优势，不等于已经解决 long-horizon execution。报告里困难的长路径和稳定空间记忆仍然会失败；长 context 也不自动等于可靠的长程策略。

目前比较突出的缺口在执行层：相邻 action 能否连贯衔接？推理等待期间怎么维持控制？能否判断接触、滑移和微小偏差？这里的 contact-state estimation 是从失败现象提出的瓶颈判断，不是报告已经单独测量并证明的认知机制。

左侧擦桌子的 demo 需要持续接触，但公开 post 没有充分的重复实验和控制细节，不能仅凭画面断言它懂得力控制。前面的插入例子也说明，完成语义上正确的计划，距离最后几毫米的可靠执行还有差距。

对于已训练分布内的精细 manipulation，专门训练的 generative action policy，例如 VLA，通常更适合承担连续动作生成。前面的 familiar LIBERO-40 甚至出现 VLA 独立运行优于 Agent supervision 的结果。但不能据此断言任意生成式模型都胜过任意 Agent；新目标上的语义适配仍可能需要上层 Agent。

所以更自然的路线是分工，而不是二选一：基础模型继续变强，interface 让它调用更合适的局部能力。最后看 RPent，怎样把这种分工落实到一个工具框架里。

## 11. RPent: Packaging Robotics as Agent Tools
...
用 RPent 收束第一章：直接 action、scripted controller 和 learned primitive 不必相互排斥。更强的 foundation model 提供更好的理解与决策；对同一个模型，更合适的 interface 则能改善它调用能力和利用反馈的方式。RPent 是这种组合思路的工程例子，不是已经证明消除全部接触和长程执行困难的结果。

第一章主要讨论执行中的 action 和 tools。如果希望减少真机反复试错，还可以让 Agent 组织工具构建 simulator，先检查候选动作。下一章转向 Agentic Real2Sim，并区分“复现已发生的交互”和“预测新动作的后果”。
