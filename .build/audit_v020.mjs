// Capture and check all four v0.20 revised pages.
...
try {
  ...
  const footers = [];
  for (const number of [31,34,35,37]) {
    ...
    footers.push(record);
    await page.locator('#slide').screenshot({path:`${root}/.build/v0_20_visual/${number}.png`});
  }
  if (await servedHash() !== html_sha256) throw new Error('Served bytes changed');
  writeFileSync(`${root}/.build/v0_20_changed_slides_audit.json`,
    JSON.stringify({html_sha256, loaded_deck_data_sha256:sha(dom), footers},null,2));
  console.log(`Four revised pages and preserved footer style verified: ${html_sha256}`);
} finally {
  await browser.close();
}
