// Verify and capture the P31 table/summary cleanup.
...
try {
  ...
  const footers = [];
  for (let number=31;number<=31;number++) {
    ...
    footers.push(record);
    await page.locator('#slide').screenshot({path:`${root}/.build/p31-cleanup.png`});
  }
  if (await servedHash() !== html_sha256) throw new Error('Served bytes changed');
  writeFileSync(`${root}/.build/v0_19_p31_layout_audit.json`,
    JSON.stringify({html_sha256, loaded_deck_data_sha256:sha(dom), footers},null,2));
  console.log(`P31 cleanup screenshot and preserved citation verified: ${html_sha256}`);
} finally {
  await browser.close();
}
