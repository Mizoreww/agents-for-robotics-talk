...
assert {r['name']:r['sha256'] for r in baseline['media']}=={r['name']:r['sha256'] for r in deck['media'] if r['name']!='asim_code'}
assert next(r['sha256'] for r in deck['media'] if r['name']=='asim_code')=='beff2d66b6b6454652c19e9229c35183eff75e026671b06749bdbaaf57d981ef'
removed_alts={
...
assert set(deck['assets'])==({r['sha256'] for r in baseline['images']}-removed_figures)|set(new_image_keys.values())|{hashlib.sha256((B/'assets/v0_13_code/asim_code.jpg').read_bytes()).hexdigest()}
...
expected_bold={7: {(1, 2), (1, 3), (0, 3)}, 10: {(0, 7), (0, 4), (2, 7), (0, 3), (0, 6), (1, 7), (0, 5), (1, 5), (2, 5)}, 21: {(0, 1), (0, 3), (1, 3), (0, 4)}, 30: {(0, 1), (1, 2)}, 31: {(2, 2)}, 33: {(0, 1), (0, 3), (2, 2)}}
...
for number,terms in {21: ['100 episodes', 'excluded'], 30: ['4 plotted traces', 'conditional retries', 'not pooled pass@1'], 31: ['40-episode', 'No reset/retry', 'not published']}.items():
...
community=[]
for name in ['clip_manifest.json','clip_manifest_v2.json','clip_manifest_v3.json','clip_manifest_v4.json','clip_manifest_v5.json']:
 community+=json.loads((B/name).read_text())
community_by_name={c['name']:c for c in community}
expected_media={2: {'painting'}, 7: {'astra_bowl', 'astra_insertion'}, 8: {'keyboard', 'policy_plug'}, 9: {'asim_waypoint', 'asim_delta', 'asim_code'}, 11: {'anon_hybrid_pack', 'anon_direct_sort'}, 18: {'ar2s_sim', 'ar2s_real'}, 22: {'astra_real2sim', 'office_newton'}, 27: {'enpire_verify', 'enpire_reset'}, 32: {'quad_rl', 'wenli_icl'}, 23: {'hand'}}
