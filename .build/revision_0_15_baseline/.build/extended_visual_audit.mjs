// Additional v0.14 verification: original-figure screenshots and whole new-clip playback.
...
try {
  ...
  const figures=[];
  for (const [n,index] of [[5,0],[5,1],[5,2],[11,0],[12,0],[12,1],[13,0],[14,0],[19,0],[24,0],[26,0],[28,0],[30,0],[31,0]]) {
    ...
    const file=`${root}/.build/v0_14_visual/figure-${n}-${index}.png`;
    await page.screenshot({path:file});
    figures.push({slide:n,index,file});
    await page.keyboard.press('Escape');
  }
  const playback=[];
  for (const [n,name] of [[8,'keyboard'],[22,'office_newton'],[32,'quad_rl'],[9,'asim_delta'],[9,'asim_waypoint'],[9,'asim_code']]) {
    await go(n);
  ...
  await go(8);await page.keyboard.press('n');
  await page.screenshot({path:`${root}/.build/v0_14_visual/script-desktop.png`});
  await page.setViewportSize({width:390,height:844});
  await page.screenshot({path:`${root}/.build/v0_14_visual/script-mobile.png`,fullPage:true});
  ...
  writeFileSync(`${root}/.build/v0_14_extended_audit.json`,JSON.stringify(report,null,2)+'\n');
