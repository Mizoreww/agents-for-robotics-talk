...
from enpire_order_v019 import RESET_CLIPS
...
class EnpireTests(unittest.TestCase):
        ...
        for old, new in zip(original, current):
            ...
            for field in ('title', 'html', 'script'):
                ...
                if old['number'] == 3 and field == 'html':
                    ...
                    # Approved P3 follow-up removes this band and changes only vertical positions.
                    expected = expected.replace('Established experiments explain mechanisms; new demos reopen the boundaries.', '')
                    self.assertEqual(re.sub('<[^>]+>', ' ', expected).split(),
                                     re.sub('<[^>]+>', ' ', new[field]).split())
                    continue
                ...
                if old.get('new_slide_key') == 'thanks' and field == 'html':
    ...
    def test_chapter_and_rsi_boundaries(self):
        ...
        self.assertEqual(len(d['slides']), 37)
        self.assertEqual({m['name'] for m in d['media'] if m['slide'] >= 23}, NEW_CLIPS | RESET_CLIPS)
