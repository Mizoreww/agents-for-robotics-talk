// Additional v0.19 verification: original-figure screenshots and whole new-clip playback.
...
mkdirSync(`${root}/.build/v0_20_visual`,{recursive:true});
...
try {
  ...
  for (let n=1;n<=37;n++) {
    await go(n);
    await page.locator('#slide').screenshot({path:`${root}/.build/v0_20_visual/${String(n).padStart(2,'0')}.png`});
  }
  const figures=[];
  for (const [n,index] of [[5,0],[5,1],[5,2],[12,0],[13,0],[13,1],[14,0],[15,0],[24,0],[27,0],[27,1],[28,0],[29,0],[30,0],[32,0]]) {
    ...
    const file=`${root}/.build/v0_20_visual/figure-${n}-${index}.png`;
    await page.screenshot({path:file});
    figures.push({slide:n,index,file});
    await page.keyboard.press('Escape');
  }
  const playback=[];
  for (const [n,name] of [[9,'cube'],[9,'claw'],[33,'enpire_pin'],[33,'enpire_gpu'],[33,'enpire_tie'],[33,'enpire_cut'],[25,'enpire_verify_full'],[26,'enpire_reset_pusht'],[26,'enpire_reset_full'],[26,'enpire_reset_tie'],[26,'enpire_reset_gpu'],[30,'enpire_fleet']]) {
    await go(n);
  ...
  await go(35);await page.keyboard.press('n');
  await page.screenshot({path:`${root}/.build/v0_20_visual/script-desktop.png`});
  await page.setViewportSize({width:390,height:844});
  await page.screenshot({path:`${root}/.build/v0_20_visual/script-mobile.png`,fullPage:true});
  ...
  writeFileSync(`${root}/.build/v0_20_extended_audit.json`,JSON.stringify(report,null,2)+'\n');
