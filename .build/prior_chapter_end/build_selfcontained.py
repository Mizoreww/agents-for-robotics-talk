...
def summary(title,num,nodes,feedback,shown,open_,setup='',reference=False):
 els=[copy.deepcopy(layouts[4][0]),text(str(num),64,57,45,48,36,'#2ba39b',True,True),text(title,110,57,965,48,36,bold=True,serif=True),rect(64,111,1152,3,'#2ba39b'),text(setup or 'Our synthesis of the chapter',80,160,1120,45,23,'#646464')]
 y=273
...
summaries={
 'exec':('Execution: Reasoning Through an Interface',summary('Execution: Reasoning Through an Interface',1,[('Goal','task and constraints'),('Agent','task-level decision'),('Controller','grounded action'),('Robot','physical execution')],'Observation and task progress','Performance depends strongly on the control interface.','Precise contact, timing and consistently helpful supervision.'),['S09']),
...
# Three chapter summaries replace the former combined comparison.
order=list(range(1,10))+['exec']+list(range(10,14))+['world']+list(range(14,19))+['improve',19,20,22]
assert len(order)==24
minutes=[.5,2.5,2.5,1,2.5,3,3,2.5,1.5,2,3,4,3.5,2.5,2,3,4.5,3.5,3.5,2.5,2,1.5,2,1.5]
...
for n,key in enumerate(order,1):
 ...
 r.pop('notes',None)
 r.update(number=n,minutes=minutes[n-1],script=script_sections[n],old_slide=key if isinstance(key,int) else None)
 r['group']='Introduction' if n<=4 else '1 · Execution' if n<=10 else '2 · World building' if n<=15 else '3 · Improvement' if n<=21 else 'Applications and conclusion'
 # Use new numbering and consistent linked source footers.
 els=[e for e in els if not(e['kind']=='shape' and e['bbox'][1]>=660)]
 if key==4:
  for e in els:
   if e.get('text')=='What makes a world testable?':e['text']='Can it build a useful simulator?'
 if key==18:
  r['title']='Faster Research, Higher Token Use'
  for e in els:
   if e.get('text')=='More Robots Shorten Time, but Raise Cost':e['text']=r['title']
 if key==22:
  els += [text('What survives the run, and does it help on a new task?',172,611,1038,39,25,'#1d7d76',True)]
 if key=='world':
...
report={'slides':24,'videos':len(new_media),'media_seconds':sum(m['duration_seconds'] for m in new_media),'minutes':sum(minutes),'chapter_summary_slides':[10,15,21],'bytes':dest.stat().st_size,'sha256':hashlib.sha256(dest.read_bytes()).hexdigest(),'standalone_file':str(dest),'media':[{k:m[k] for k in ['name','slide','sha256','duration_seconds']} for m in new_media],'images':image_audit,'native_text':True,'all_fonts_embedded':True}
