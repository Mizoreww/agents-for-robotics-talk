...
| 05 | Claude Plays Robotics | 2.5 | 06:30 |
| 06 | Direct Control Meets Physical Constraints | 3 | 09:00 |
| 07 | A Supervisor Can Disrupt a Capable VLA | 3 | 12:00 |
| 08 | Demo: Placement and Fine Insertion | 2.5 | 15:00 |
| 09 | Demo: Wiping a Table | 1.5 | 17:30 |
| 10 | Execution: Reasoning Through an Interface | 2 | 19:00 |
| 11 | Agentic Real2Sim: A Real Episode and Its Twin | 3 | 21:00 |
| 12 | Agentic Real2Sim: The Conversion Pipeline | 4 | 24:00 |
| 13 | Replay Acceptance on DROID-100 | 3.5 | 28:00 |
| 14 | Demo: Real2Sim and a Disclosed Failure | 2.5 | 31:30 |
| 15 | World Building: Replay and Prediction | 2 | 34:00 |
| 16 | ENPIRE: Real Experiments Inside the Loop | 3 | 36:00 |
| 17 | ENPIRE: Environment and Improvement | 4.5 | 39:00 |
| 18 | ENPIRE: Reset and Verification | 3.5 | 43:30 |
| 19 | ENPIRE: Improvement on Pin Insertion | 3.5 | 47:00 |
| 20 | Faster Research, Higher Token Use | 2.5 | 50:30 |
| 21 | Improvement: A Repeatable Real Experiment | 2 | 53:00 |
...
第一个标杆从一个容易被忽略的问题开始：我们说“让语言模型控制机器人”，到底给了模型什么控制权？
...
它仍然很好地说明了 Agent 的潜在用途：通过现成工具，协调一个实际任务。接下来，我们用一张图把 execution 这一章收束起来。
...
> 播放与指图提示：从左到右讲四个框，再沿 observation 的反馈箭头返回 Agent。最后用底部 Evidence 和 Still open 两句话收束本章。

在 execution loop 里，Agent 改的是一个决策。Controller 或 skill 把它变成可执行动作，机器人产生新的 observation，再影响下一次决策。

Interface 决定了哪些难题由 Agent 解决，哪些由 controller 接管。一个 move-to tool、一段生成的控制程序，以及一个 VLA，提供的支持并不相同。第一个标杆说明，解释模型表现之前，需要先把这些支持条件讲清楚。

现有证据支持 general reasoning 在合适执行接口上发挥作用，同时也保留了几个明显的问题：precise contact、timing，以及 supervision 是否持续有益。成功的视频不会自动消除这些问题。

这时我们自然会想到：能不能少在真实机器人上反复试错？如果有一个 simulator，能表达相关的交互，我们或许可以先检查场景，再测试替代动作。

这就引出了下一章。但 simulator 要能帮助决策，它就必须把相关物理后果预测对。接下来要看的，是 Agent 能从一段录制的交互里构建出多少能力，以及怎样区分“回放得像”和“预测得准”。
...
所以这一章会按三个步骤展开。先看 Agent 如何协调 conversion pipeline，再看论文所谓 replay acceptance 的具体定义，最后通过一个独立 demo 中公开的失败案例，理解物理表示的限制。
...
现在可以把 world-building 这一章整理成一张反馈图。
...
> 播放与指图提示：分别追踪 recording 到 Agent、recording 到 comparison 的两条路径，再讲 mismatch 的返回箭头。强调 reference 与 feedback 的角色不同。

这一章里，Agent 改的是 scene 或 model，不再是机器人下一步的动作。Agent 和工具创建 candidate，simulator 运行它，再与真实 recording 比较，产生下一轮修正的依据。

图上方的 reference path 很关键。系统当前试图解释的是一个已经观察到的 interaction。如果评价仍然只围绕这个 interaction，那么我们检验的主要是 reconstruction consistency。

Agentic Real2Sim 的证据说明，这个流程可以在一批完整尝试中产生被接受的 replay。最好的 observed backend 是 48/100，同时它也明确保留了失败案例。

尚未自动解决的是 prediction：一旦 action 变了，场景还是否有用？这个问题需要超出拟合所用 recording 的证据。

于是，如果我们希望进一步改进机器人，有两条可以选择的路线：加强 simulator，并检验它对新动作的预测；或者直接从真实机器人获取反馈。后一种方式不依赖重建世界的预测精度，但会带来真实实验的成本和组织难题。

这就是下一章 ENPIRE 的动机：需要什么基础设施，Agent 才能反复运行真实实验，并把结果转成有效的 policy improvement？
...
## 20. Faster Research, Higher Token Use
...
> 播放与指图提示：沿 fixed API、policy changes、real rollout、verifier/logs 讲一遍，再追踪反馈箭头。章节结论讲清楚后，再转到两个简短应用 demo。

这一章里，Agent 改的是 policy 或 training procedure。真实 rollout 返回证据，verification 和 log 帮助 Agent 判断这个改动是否值得保留，以及下一步该检查什么。

Fixed environment contract 让这个过程具有可解释性。改 policy 的时候，action、reset 和 success signal 的定义保持稳定，才容易判断性能变化来自哪里。

ENPIRE 说明，在它定义的 protocol 下，这种安排能够推动策略改善。边界同样很清楚：human-assisted setup、reset quality、verifier error、conditional retries，以及能否 transfer 到已测试条件之外。

现在我们可以比较三个章节，又不必比较它们的百分比。在 execution 中，Agent 改决策；在 world building 中，Agent 改 simulated scene；在 improvement 中，Agent 改 policy 或 training procedure。它们得到的 feedback 不同，产生的 artifact 也不同。

结束前，再看两个很短的应用例子。它们不构成第四个详细标杆，而是让我们看看 Agent 还能创建或保留哪些东西，并把同样的证据问题应用到机器人控制之外。
