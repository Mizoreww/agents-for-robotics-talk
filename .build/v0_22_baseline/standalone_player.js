...
// Fit the stage to the actual left column, including wrapped controls.
function resize(){
 ...
 const controls=[document.querySelector('.toolbar'),$('message'),document.querySelector('.help')]
  .filter(e=>getComputedStyle(e).display!=='none');
 const availableWidth=Math.max(0,main.clientWidth-paddingX);
 const availableHeight=main.clientHeight-paddingY-controls.reduce((h,e)=>h+e.getBoundingClientRect().height,0)
  -controls.length*parseFloat(css.gap);
 const width=matchMedia('(max-width:700px)').matches?availableWidth:
  Math.min(availableWidth,Math.max(0,availableHeight)*16/9);
 $('stage').style.width=width+'px';
 $('slide').style.transform='scale('+width/1280+')';
}
const layoutObserver=new ResizeObserver(resize);
for(const e of [document.querySelector('main'),document.querySelector('.toolbar'),$('message'),document.querySelector('.help')])layoutObserver.observe(e);
...
async function show(number){
 ...
 notes(s);if(!allScript)$('notes-scroll').scrollTop=0;resize();$('prev').disabled=current===1;$('next').disabled=current===deck.slides.length;
...
let scriptOpen=false,scriptCloseTimer;
function setScriptOpen(open){
 clearTimeout(scriptCloseTimer);scriptOpen=open;
 const panel=$('notes'),toggle=$('notes-toggle');
 if(open){
  panel.hidden=false;
  // Establish the closed geometry before transitioning the grid track.
  void panel.offsetWidth;
 }
 panel.inert=!open;panel.setAttribute('aria-hidden',String(!open));
 document.body.classList.toggle('script-open',open);
 toggle.setAttribute('aria-expanded',String(open));
 toggle.querySelector('span').textContent=open?'Hide script':'Speaker script';
 if(open)$('notes-close').focus({preventScroll:true});
 else{
  ...
  scriptCloseTimer=setTimeout(()=>{if(!scriptOpen)panel.hidden=true},delay);
 }
}
$('notes-toggle').onclick=()=>setScriptOpen(!scriptOpen);
$('notes-close').onclick=()=>setScriptOpen(false);
$('all-script').onclick=()=>{
 allScript=!allScript;
 $('all-script').querySelector('span').textContent=allScript?'Current slide':'Full script';
 $('all-script').setAttribute('aria-pressed',String(allScript));
 notes(deck.slides[current-1]);$('notes-scroll').scrollTop=0;
};
...
document.addEventListener('keydown',e=>{
 ...
 else if(e.key==='Home'&&!$('notes').contains(e.target)){e.preventDefault();show(1)}else if(e.key==='End'&&!$('notes').contains(e.target)){e.preventDefault();show(deck.slides.length)}
 else if(e.key===' '&&e.target.tagName!=='BUTTON'&&!$('notes').contains(e.target)){e.preventDefault();videos().some(v=>!v.paused)?pause():play()}
