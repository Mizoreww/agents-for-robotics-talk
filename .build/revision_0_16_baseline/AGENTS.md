...
One self-contained HTML technical talk — 33 English slides, Chinese speaker script, 18 embedded clips,
~130 MB — plus its research notes and build scripts. Speaker Zimo Huang, talk date 2026-09-16.
...
- Control follows the newly approved sequence: original overview; Hi Robot / Helix / Helix 02 on one
  ...
  Direct/Hybrid evidence; summary. Data is demo-only: Office scene, tendon-hand CAD,
  multi-view replay and DexGPT rollout, all discovered through Awesome-Astra. No Agentic Real2Sim paper pages.
...
Version 0.15 (2026-09-15): Control 4–16, Data 17–22, Improvement 23–31, closing 32–33.
33 slides, 18 clips, 50.5-minute suggested timing. HTML SHA-256 `e3b2d420b01fe153c34ef0336b41b16d11dbb6aa9ea20027c14ef571e0fbd210`.

Current scope: `research/revision_0_15_spec.md`. Fixed comparison is `.build/revision_0_15_baseline/`,
not older Git HEAD. Chapter spine: `output/Chapter_Spine.md`. Overview diagrams open 4/17/23;
three original hierarchy figures share 5; responsibility shift replaces Hi Robot results on 6; report architecture is 11; original results are 12–14; ENPIRE idea tree is 26.
...
Code replay authority: `research/code_video_v0_14_sources.json`, checked by `.build/code_video_v014.py`. v0.15 removes only ar2s_real/ar2s_sim and adds the original DexGPT replay; all retained clip bytes are unchanged.
...
`.build/v0_15_extended_audit.json` for actual hash-bound status. v0.15 QA/review passed; Standards0 / Spec0, no unresolved findings; see `research/revision_0_15_review.md`. Prior v0.9 source-freeze P2 remains resolved.
...
Current presentation polish: merged learned tools on 4; Astra begins on 7; capability summary on 16; Takeaways on 32 and Thank You on 33. Native table emphasis is tested per cell; resources and retained slots are descriptive, not an overall winner.

Data now uses four demos: Office (18), hand CAD (19), multi-view replay (20), and DexGPT rollout (21), followed by a summary (22). DexGPT is a failed physical-acceptance example, not a paper deep dive. Control timing (15) separates Asim published query seconds from controller Hz and Go1 simulation-time feedback.

P15 is now latency-only (ASIM); P16 has four authored inline SVG icons and two open problems. Page-number typos in user edits are resolved by the exact quoted text. P10 Waypoint proprio is a highlighted column, not a universal winner; Can delta18/20 exceeds waypoint17/20. Current polish/build scope is v0.15.
