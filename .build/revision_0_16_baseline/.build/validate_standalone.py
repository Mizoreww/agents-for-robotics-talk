...
deck=json.loads(p.scripts['deck-data']);N=33;V=18
...
assert [r['group'] for r in deck['slides']]==['Introduction']*3+['1 · Agent Controls Robot']*13+['2 · Agent Creates Data']*6+['3 · Agent Improves Policy']*9+['Closing']*2
expected_titles=['Agents for Robotics', 'Demo: Painting with Feedback', 'Three Roles for Robotics Agents', 'Agent Controls Robot', 'Hierarchical Robot Control', 'Where Does Generalization Live?', 'Astra: Stronger Real-Robot Performance', 'Astra: More Real-Robot Demos', 'One Model, Different Control Interfaces', 'Astra Interfaces: Three Tasks', 'Astra: Introducing a Hybrid Architecture', 'RoboDojo: Overall Results', 'RoboLab: Overall Results', 'RoboDojo: Task-Level Patterns', 'High Latency Limits Real-Time Control', 'Control: Strengths and Open Gaps', 'Agent Creates Data', 'Astra: From Office Scan to Simulation', 'Data: Mechanism and CAD Assets', 'Astra: Real-to-sim Replay', 'Astra: Dexterous Data Rollout', 'Data: From Demos to Useful Data', 'Agent Improves Policy', 'ENPIRE: Environment and Improvement', 'ENPIRE: Reset and Verification', 'ENPIRE: What Did the Agent Change?', 'ENPIRE: Pin Insertion Curve', 'ENPIRE: Two Physical Tasks', 'ENPIRE: Autoresearch in RoboCasa', 'Astra: Training and Context', 'ENPIRE: Cost and Chapter Summary', 'Takeaways', 'Thank You']
...
for n in [23, 24, 25, 26, 27]:assert slide(n)['ids']==['S15']
...
assert all(t in slide(26)['script'] for t in ['I37','10.8','I66','I76','不是三项独立'])
assert 'one-shot' in slide(27)['script'] and 'tokens' in slide(31)['script']
assert all(t in slide(30)['footer'] for t in ['4 experts','FULL EVAL NOT MET','8×'])
...
assert len(re.sub('<[^>]+>',' ',slide(32)['html']).split())<65
assert all(t in slide(32)['html'] for t in ['Takeaways','Reliable execution','Physics + training value','Held-out gain + cost'])
assert '不是一条已打通' in slide(32)['script']
assert 'Thank you' in slide(33)['html'] and 'Questions &amp; discussion' in slide(33)['html']
assert len(re.sub('<[^>]+>',' ',slide(33)['html']).split())<25
...
assert all(term in slide(22)['html'] for term in ['Assets / scenes','Real-to-sim Replay','Data Rollout','Does it improve downstream training?'])
...
# v0.15 removes only the retired paper's two clips; every retained payload is unchanged.
baseline=json.loads((B/'revision_0_15_baseline/.build/standalone_build_audit.json').read_text())
expected_retained={r['name']:r['sha256'] for r in baseline['media'] if r['name'] not in {'ar2s_real','ar2s_sim'}}
assert expected_retained=={r['name']:r['sha256'] for r in deck['media'] if r['name']!='dexgpt'}
from data_demos_v015 import verify_data_demos
verify_data_demos(ROOT)
assert next(r['sha256'] for r in deck['media'] if r['name']=='dexgpt')=='a44512d59d55f5eeb20e22138c64cea585bcc692b0ba5b98ddb5eba4b07c403a'
...
assert [r['new_slide_key'] or r['old_slide'] for r in deck['slides'][16:22]]==['world','data_scene',19,'data_replay','data_rollout','world_summary']
...
old_text=(B/'revision_0_15_baseline/output/Agents_for_Robotics_Self_Contained.html').read_text()
old_deck=json.loads(re.search(r'<script type="application/json" id="deck-data">(.*?)</script>',old_text,re.S)[1])
# Exact asset closure: unchanged chapters, hand assets, retained demo posters, and new DexGPT poster.
kept_slides=old_deck['slides'][:16]+[old_deck['slides'][22]]+old_deck['slides'][24:]
expected_assets={k for r in kept_slides for k in re.findall(r'data-asset="([a-f0-9]+)"',r['html'])}
expected_assets|={m['posterKey'] for m in old_deck['media'] if m['name'] not in {'ar2s_real','ar2s_sim'}}
expected_assets.add(hashlib.sha256((B/'assets/v0_15/dexgpt.jpg').read_bytes()).hexdigest())
...
table_cells(28,['Coding agent','Push-T @ 8 h\nNormalized score','Pin @ 4 h\nSuccess rate'],
 [['Codex','0.938','95.5%'],['Claude','0.750','97.5%'],['Kimi','0.625','79.0%']])
table_cells(29,['Method','What changes','Reported evidence'],
...
table_cells(31,['Agent–robot pairs','Mean robot utilization','Mean GPU utilization','Mean tokens / minute'],
...
expected_bold={7: {(1, 2), (1, 3), (0, 3)}, 10: {(0, 7), (0, 4), (2, 7), (1, 5), (0, 3), (0, 6), (1, 7), (0, 5), (2, 5)}, 28: {(0, 1), (1, 2)}, 29: {(2, 2)}, 31: {(0, 1), (0, 3), (2, 2)}}
...
for number,terms in {28: ['4 plotted traces', 'conditional retries', 'not pooled pass@1'], 29: ['40-episode', 'No reset/retry', 'not published']}.items():
...
community=[]
for name in ['clip_manifest.json','clip_manifest_v2.json','clip_manifest_v3.json','clip_manifest_v4.json','clip_manifest_v5.json','clip_manifest_v6.json']:
 community+=json.loads((B/name).read_text())
community_by_name={c['name']:c for c in community}
expected_media={2: {'painting'}, 7: {'astra_insertion', 'astra_bowl'}, 8: {'policy_plug', 'keyboard'}, 9: {'asim_code', 'asim_waypoint', 'asim_delta'}, 11: {'anon_hybrid_pack', 'anon_direct_sort'}, 25: {'enpire_verify', 'enpire_reset'}, 30: {'wenli_icl', 'quad_rl'}, 19: {'hand'}, 18: {'office_newton'}, 20: {'astra_real2sim'}, 21: {'dexgpt'}}
...
assert [r['slide'] for r in ui['addedFigures']]==[11,12,13,14,24,26,28,29] and all(r['open'] and r['loaded'] for r in ui['addedFigures'])
