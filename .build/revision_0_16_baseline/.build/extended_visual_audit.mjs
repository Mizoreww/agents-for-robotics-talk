// Additional v0.15 verification: original-figure screenshots and whole new-clip playback.
...
try {
  ...
  const figures=[];
  for (const [n,index] of [[5,0],[5,1],[5,2],[11,0],[12,0],[12,1],[13,0],[14,0],[24,0],[26,0],[28,0],[29,0]]) {
    ...
    const file=`${root}/.build/v0_15_visual/figure-${n}-${index}.png`;
    await page.screenshot({path:file});
    figures.push({slide:n,index,file});
    await page.keyboard.press('Escape');
  }
  const playback=[];
  for (const [n,name] of [[18,'office_newton'],[19,'hand'],[20,'astra_real2sim'],[21,'dexgpt']]) {
    await go(n);
  ...
  await page.screenshot({path:`${root}/.build/v0_15_visual/script-desktop.png`});
  await page.setViewportSize({width:390,height:844});
  await page.screenshot({path:`${root}/.build/v0_15_visual/script-mobile.png`,fullPage:true});
  ...
  writeFileSync(`${root}/.build/v0_15_extended_audit.json`,JSON.stringify(report,null,2)+'\n');
