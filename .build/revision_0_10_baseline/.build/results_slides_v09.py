...
def make_results(root, page, text, rect, figure, band):
    ...
    def table(headers, rows, widths, y=210, row_h=43, head_h=48, size=22, x=64):
        ...
        for index, row in enumerate([headers] + rows):
            ...
            for col, (cell, width) in enumerate(zip(row, widths)):
                els += [text(str(cell), left + 10, top + 4, width - 20, height - 7,
                             size, '#1d7d76' if index == 0 else '#1a1a1a',
                             index == 0, align='left' if col == 0 else 'center')]
                left += width
        return els
    ...
    els += [text('Square is sensitive to both interface and observations', 74, 516, 1132, 38, 27, '#1d7d76', True)]
    els += note('Unequal budgets: ΔEEF ≤200 steps / 10 queries; Square waypoint ≤500 / 16; code ≤500 / 3 revisions.\n“Proprio” includes visual markers and fixed references. ΔEEF means OSC_POSE, not joint q.', 585, 18)
    ...
    els += table(['Method', 'What changes', 'Reported evidence'],
                 ...
                 [270, 488, 394], y=488, row_h=29, head_h=32, size=19)
