...
对应新版 26 页 HTML。幻灯片为英文，讲稿为中文，论文名、模型名和技术术语保留英文。正文可直接讲；“播放与指图提示”不念出。建议总时长约 60 分钟，包含视频、读图和短互动；并非要求将正文逐字匀速读满 60 分钟。正式演讲前建议按自己的语速彩排一次。
...
| 02 | Demo: A Robot Paints with Feedback | 2 | 00:30 |
| 03 | Our Ultimate Goal | 2.5 | 02:30 |
| 04 | Three Core Roles, Broader Applications | 1 | 05:00 |
| 05 | Execution: An Agent with Many Tools | 2.5 | 06:00 |
| 06 | RPent: Packaging Robotics as Agent Tools | 2.5 | 08:30 |
| 07 | Claude Plays Robotics | 3 | 11:00 |
| 08 | Direct Actions and Controller Code | 3 | 14:00 |
| 09 | VLA Tools: Grounding and Supervision | 2.5 | 17:00 |
| 10 | Demo: Placement and Fine Insertion | 2 | 19:30 |
| 11 | Demo: Wiping a Table | 1 | 21:30 |
| 12 | World Building: Replay and Prediction | 2 | 22:30 |
| 13 | Agentic Real2Sim: A Real Episode and Its Twin | 2.5 | 24:30 |
| 14 | Agentic Real2Sim: The Conversion Pipeline | 3.5 | 27:00 |
| 15 | Replay Acceptance on DROID-100 | 3 | 30:30 |
| 16 | Demo: Real2Sim and a Disclosed Failure | 2 | 33:30 |
| 17 | Improvement: A Repeatable Real Experiment | 2 | 35:30 |
| 18 | ENPIRE: Real Experiments Inside the Loop | 3 | 37:30 |
| 19 | ENPIRE: Environment and Improvement | 4 | 40:30 |
| 20 | ENPIRE: Reset and Verification | 3.5 | 44:30 |
| 21 | ENPIRE: Improvement on Pin Insertion | 3 | 48:00 |
| 22 | Faster Research, Higher Token Use | 2 | 51:00 |
| 23 | Other Applications: Design and Engineering | 1.5 | 53:00 |
| 24 | Demo: Structural Design of a Robot Hand | 2 | 54:30 |
| 25 | Demo: Inspecting and Repairing an Execution | 2 | 56:30 |
| 26 | Conclusion | 1.5 | 58:30 |
...
前三章围绕三个标杆展开：Claude Plays Robotics 看执行接口，Agentic Real2Sim 看构建仿真场景，ENPIRE 看真实实验中的策略改进。第四章再拓展到结构设计、程序调试等其他应用。我们既看这些组合带来的能力，也看它们实际产生了什么，以及怎样验证这些产物。
...
> 播放与指图提示：沿 Human goal → Agent + Tools → Useful artifacts 讲，再回到反馈箭头。这里的 artifact 不只指训练好的 policy，也包括代码和设计。

这张图先给出整个演讲的视角：人提出目标，Agent 选择并组织工具，执行过程中根据反馈继续调整。我们希望得到的，是对 robotics 真正有用的结果。

第一层价值是把当前工作完成。Agent 可以让 IK 处理 pose 到 joint 的转换，让 controller 承担快速反馈，让 VLA 提供已经学到的动作能力。这样，general reasoning 就有了与物理世界交互的具体接口。

第二层价值是保留工作成果。它可以是一段修好的程序、一个可复用的 skill、一个 simulated scene、训练后的 policy，也可以是一份 mechanical design。这些产物不全是 model weights，也不全都直接控制机器人。

不同产物需要不同的验证。程序要运行测试；scene 要检查对新动作的预测；policy 要在没有参与改进的条件下评测；mechanical design 则还要经过制造、装配和物理实验。生成了一个 artifact，不等于已经证明它能可靠使用。

这张图是我们的综合视角，不是某篇论文已经实现的统一架构。后面的四章会用具体案例说明：Agent 组织了哪些工具，工具承担了什么工作，结果应该怎么评价。


## 04. Three Core Roles, Broader Applications

建议 1 分钟（含读图、视频及停顿）。

第一章是 execution through tools：Agent 接到不同的机器人接口上，会获得什么能力？Claude Plays Robotics 很适合回答这个问题，因为它系统比较了多种控制方式。

第二章是 world building：Agent 能否组织 perception、geometry 和 simulation 工具，把真实交互转成可运行的 scene？我们用 Agentic Real2Sim 展开。

第三章是 improvement：如果 Agent 能反复调用真实机器人实验，它能不能改进 policy？ENPIRE 展示了为此需要的环境接口和实验基础设施。

第四章是 other applications。Robotics 不只有执行、建模和训练，还包括结构设计、软件调试以及很多工程工作。这里会用手的设计和 ASPIRE 的调试 demo 看两种不同的产物，不再增加第四篇细讲论文。

这些章节按主要工作对象组织，不是互斥的完整分类，也不是必须依次执行的流水线。同一个 Agent 可以跨越几个角色。例如 ENPIRE 不必先运行 Agentic Real2Sim，调试也可能发生在控制和训练的任何环节。不同案例的 success rate 含义不同，我们不做跨论文百分比排行榜。


## 05. Execution: An Agent with Many Tools

建议 2.5 分钟（含读图、视频及停顿）。

> 播放与指图提示：先指 Agent，再沿分支逐个看 Agent Tools；强调工具是可选、可组合的。右侧只汇集 action outputs，底部反馈经过 perception/state tools 返回 Agent。

第一章的核心是这张图：Agent 后面不是只有一个固定 controller，而是一组可以选择和组合的 Agent Tools。它先理解目标，再选择合适的接口，调用工具并检查结果。图中的分支不是要求所有工具每次都运行，也不表示它们可以互相替代。

IK 和 motion planning 把任务空间里的目标转成机器人能执行的运动。好处是 Agent 可以围绕 end-effector pose 或 waypoint 思考，不必逐关节求解。前提是坐标、标定和机器人模型可信；IK 本身也不保证避障、接触稳定或任务成功。

Controller code 利用 Agent 的编程能力。Agent 写出反馈逻辑，runtime 负责较快地执行，让慢速 reasoning 和快速 control 分工。VLA 或其他 learned policy 则复用训练得到的动作能力：Agent 提供目标或选择指令，tool 提供 grounded action。它的优势来自已有技能，边界也取决于训练分布和接口设计。

WAM 这里指 World Action Model。以 DreamZero 为例，它联合生成未来 video 和 action，用 video pretraining 提供的知识支持行为生成。把这类模型作为 Agent 可调用的 action tool，是这里的架构视角；并不是说 Claude Plays Robotics 已经测试了这个组合，也不是说生成的视频已经具有可验证的物理预测精度。

底部还需要 perception 和 state tools，把 observation 变成有用的 pose、depth 或 heading 信息。工具既能执行动作，也能补充 Agent 缺少的信息。Training、planning 等其他服务也可以纳入这个集合，图中没有穷尽所有可能。

接下来先看 RPent 这个具体的工具组织框架，再用 Claude Plays Robotics 实际测试的接口解释分工为什么有用。IK 的真机例子后面会出现；WAM 只作为扩展示例，不在这里新增一篇论文细讲。
...
## 06. RPent: Packaging Robotics as Agent Tools
...
RPent 的全称是 Recursive Physical Agent，是 RLinf 组织下独立的 embodied-agent framework。RLinf 本体侧重 reinforcement learning infrastructure；RPent 更直接回答前一页的问题：怎样把多种机器人能力交给一个通用 Agent 调用？
...
这些工具包回答“怎样把能力接进来”。接下来的 Claude Plays Robotics 则帮助我们回答另一半问题：不同的接入方式，实际提供了什么优势，又引入了哪些限制？
...
- [ROS-MCP Server · MCP–ROS bridge](https://github.com/robotmcp/ros-mcp-server)

## 07. Claude Plays Robotics

建议 3 分钟（含读图、视频及停顿）。

> 播放与指图提示：保留原图完整的 axes 和 legend。按 direct control、programmatic control、policy control、RL supervision 讲各自提供的能力，不把叠加分数读成单一任务成功率。

这份报告最值得借鉴的地方，是把“模型怎么接到机器人上”当成实验变量。同一个 general model，接口不同，能发挥的能力也会不同。我们先看每种接口替 Agent 提供了什么。

Direct control 给模型较细的动作决定权，适合检验它如何从状态推导动作，也适合讨论那些现成 skill 没有覆盖的修正。但 direct 并不总是 joint torque：locomotion 里有低层力矩控制，manipulation 里则使用七维 end-effector motion commands。它们仍依赖各自的机器人执行栈，不能一概描述成模型直接控制全部电机。

Programmatic control 把代码作为动作接口。Agent 可以写 controller(obs) → action，让一个可检查、可修改的程序承担快速反馈。优势是复用编程能力，并把生成程序和执行程序放在不同时间尺度上。报告中不少任务通过 code control 比直接选动作更容易，但这并不意味着任意生成代码都能稳定控制机器人。

Policy control 复用预训练能力。对于 Go2，Agent 可以发 forward、lateral、yaw velocity command，让 gait policy 负责协调腿部；对于 manipulation，MolmoAct 提供 action proposal，Agent 决定接受、编辑还是替换。它减少了 Agent 从头解决 balance 或视觉动作映射的负担，让模型可以围绕目标和进度工作。代价是必须理解 tool 的适用范围，不能把原本正确的动作随意改坏。

RL supervision 则把训练过程也变成工具。Agent 可以定义 reward、policy network 和 training schedule，再训练并部署 policy。它为难以手写的行为提供了从 interaction 中获得技能的路径，也能留下可重复执行的 policy；不过需要训练预算、可信 reward 和稳定的环境。报告中 RL 并没有普遍优于 code control，因此它不是更高级就一定更好的接口。

除了 action tools，信息接口也很重要。报告中的 compass 和 cursor query 在对应任务中能提供有效帮助，而 depth overlay 等额外输入并没有普遍带来收益。这里的经验是：给出当前决策缺少的、可直接使用的信息，可能比单纯给更多图像更有效。

最后读一下这张原图的口径：柱子按 model 排列，颜色区分 interface，纵轴是汇总 embodiment score，不是某个单任务的 success rate。我们从中带走的是接口选择对整体能力的影响，而不是一个脱离 robotics stack 的模型排名。

### 参考来源

- [Anthropic, Claude Plays Robotics (9 Jul 2026)](https://www.anthropic.com/research/claude-plays-robotics)

## 08. Direct Actions and Controller Code
...
## 09. VLA Tools: Grounding and Supervision
...
## 10. Demo: Placement and Fine Insertion
...
这一章的结论是：Agent 的价值可以来自组织一组互补的 tools，而不必包办底层控制。IK 提供几何接口，controller code 提供快速反馈，learned policy 提供已有技能，perception tool 提供有用状态；选择合适的组合，才能把 general reasoning 转成实际能力。Precise contact、timing 和 supervision 的收益仍要在具体任务里检验。
...
## 12. World Building: Replay and Prediction

建议 2 分钟（含读图、视频及停顿）。

> 播放与指图提示：分别追踪 recording 到 Agent、recording 到 comparison 的两条路径，再讲 mismatch 的返回箭头。先建立结构，不在这里展开 acceptance 的评分细则。

第二章先看这张流程图。第一章把 WAM 视为可调用的行为模型；这里的工作对象则是从真实记录构建一个可运行的 simulated scene，两者不能因为都涉及 world 就混为一谈。与上一章修改机器人下一步的决策不同，这里 Agent 修改的是 scene 或 model。Agent 和工具创建 candidate，simulator 运行它，再与真实 recording 比较；两者的 mismatch 成为下一轮修正的依据。
...
## 13. Agentic Real2Sim: A Real Episode and Its Twin
...
## 14. Agentic Real2Sim: The Conversion Pipeline
...
## 15. Replay Acceptance on DROID-100
...
## 16. Demo: Real2Sim and a Disclosed Failure
...
## 17. Improvement: A Repeatable Real Experiment
...
## 18. ENPIRE: Real Experiments Inside the Loop
...
## 19. ENPIRE: Environment and Improvement
...
## 20. ENPIRE: Reset and Verification
...
## 21. ENPIRE: Improvement on Pin Insertion
...
## 22. Faster Research, Higher Token Use
...
前三章分别围绕执行中的工具调用、simulated scene 和 policy improvement 展开。但 robotics workflow 远不止这些。下一章我们把视野再打开一点，看 Agent 怎样参与结构设计和程序调试。

### 参考来源

- [ENPIRE, arXiv:2606.19980v1 (Jun 2026)](https://arxiv.org/html/2606.19980v1)

## 23. Other Applications: Design and Engineering

建议 1.5 分钟（含读图、视频及停顿）。

> 播放与指图提示：沿 engineering goal、Agent + Tools、artifact、validation 讲。这里只建立第四章的共同视角，不把两个 demo 扩展成新的 benchmark 综述。

第四章看前三种核心角色之外的其他应用。机械结构怎么设计，执行程序哪里出错，实验软件怎么维护，这些也是 robotics 的重要工作。

图中，Agent 面对一个 engineering goal，调用 CAD、代码编辑或分析工具，产生 design 或 program，再用相应测试检查结果。反馈仍然重要，但产物不一定是机器人当前的 action，也不一定是一个 simulator 或训练后的 policy。

手的例子可以明确归为 structural design：Agent 帮助提出机构与几何方案。ASPIRE 展示 software debugging：利用机器人执行时的观察定位问题，再修复程序。Debugging 也可能服务于前三章，因此这里不是一套互斥分类，而是把被“控制机器人”这个说法遮住的工程应用单独拿出来看。

同样的视角还可以扩展到 data curation、experiment tooling、test generation 等工作。今天不继续增加案例，只用这两个 demo 说明：应用空间更广，但验证方式必须跟着产物变化。
...
## 24. Demo: Structural Design of a Robot Hand

建议 2 分钟（含读图、视频及停顿）。

> 播放与指图提示：播放 CAD animation。将它介绍为 structural design application，并始终保留 “Not a validated physical hand”。可以请观众提出一个下一步必须做的工程检查。

这就是 Agent 对机器人结构设计的应用。这里的任务不是让一只现成机械手执行动作，而是借助 CAD 和 visualization 提出一份 hand design，展示生成的 geometry 和 animation。
...
所以第四章的第一个例子留下的是设计方案。下一段 demo 换一种产物：经过检查和修复的执行程序。
...
## 25. Demo: Inspecting and Repairing an Execution

建议 2 分钟（含读图、视频及停顿）。

> 播放与指图提示：播放约 30 秒的 ASPIRE trace-inspection 片段，指出 observation 和 program repair 的对应关系。保持 demo 深度，不扩展成完整论文分析。

ASPIRE 展示 Agent 对机器人软件调试的应用，留下的是修好的代码和 reusable skills。它可以支持 execution，但这里关注的工作对象是程序本身。
...
第四章可以总结为：Agent 的应用对象也可以是机器人的结构和软件，而不只是 action、scene 或 policy。Hand design 需要工程和物理验证，program repair 需要运行测试与后续任务验证。还有很多其他应用可以沿着这个视角展开，但不能把生成了一个产物和证明它有用混为一谈。
...
第一章的 Claude Plays Robotics 说明，选择合适的接口可以发挥已有 robotics stack 的优势：controller 承担快速反馈，learned policy 提供 grounded action，信息工具补充可用状态。Agent 的职责是组织、调用和判断，不必从头实现所有这些能力。

第二章的 Agentic Real2Sim 把这些组织能力用在构建 simulated scene 上，同时提醒我们 replay acceptance 和 predictive validity 是两种不同的验证。第三章的 ENPIRE 则把真实实验组织成改进流程；策略收益需要连同 fixed API、reset、verification 和 retry protocol 一起理解。

第四章进一步说明，结构设计、软件调试和其他工程工作也都是应用空间。一个 CAD proposal、一段 repaired program，与一次成功动作有不同的价值，也需要不同的测试。
