...
def execution_tools():
 ...
 els += [text('Our synthesis · IK and WAM extend the report’s tested interfaces',80,148,1120,38,22,'#646464'),text('Agent Tools',500,193,350,32,22,'#1d7d76',True)]
 els += [text('Task + observations',112,250,276,34,22,align='center'),connector(250,286,250,332),rect(140,332,220,88,'#2ba39b'),text('Agent',148,346,204,34,27,'#ffffff',True,align='center'),text('choose and compose',148,385,204,28,18,'#e2f3ef',align='center')]
 ...
 for i,(label,benefit) in enumerate(tools):
  y=236+i*77;centers.append(y+31)
  els += [rect(500,y,350,62,'#f4f6f5','#c6d9d6'),text(label,511,y+5,328,29,22,bold=True),text(benefit,511,y+34,328,25,17,'#646464')]
  els += [connector(412,y+31,500,y+31),connector(850,y+31,939,y+31,arrow=False)]
 els += [connector(360,376,412,376,arrow=False),connector(412,centers[0],412,centers[-1],arrow=False),connector(939,centers[0],939,centers[-1],arrow=False),connector(939,376,1010,376),text('Action outputs',984,284,233,32,19,'#646464',align='center')]
 els += [rect(1010,332,190,88,'#f4f6f5','#c6d9d6'),text('Execution',1018,346,174,34,24,bold=True,align='center'),text('robot / runtime',1018,385,174,28,18,'#646464',align='center')]
 # Feedback is observed state, not an assertion that every tool emits motor actions.
 els += [connector(1105,420,1105,584,'#2ba39b',False),connector(1105,584,850,584,'#2ba39b'),text('Observations',878,541,213,30,19,'#1d7d76',align='center'),rect(500,554,350,62,'#edf7f4','#c6d9d6'),text('Perception / state tools',511,559,328,29,22,'#1d7d76',True),text('pose · depth · heading',511,588,328,25,17,'#646464'),connector(500,584,250,584,'#2ba39b',False),connector(250,584,250,420,'#2ba39b')]
 els += [text('Each tool handles a different part of the robotics problem.',80,632,1120,32,23,'#1d7d76',True)]
 return els
...
def agenda():
 ...
 rows=[('Execution through tools','Claude Plays Robotics'),('World building','Agentic Real2Sim'),('Improvement through experiments','ENPIRE'),('Other applications','Structural design · debugging · engineering tools')]
...
def conclusion():
 ...
 rows=[('Execution: combine tools at the right abstraction.','IK, controllers and learned policies provide different capabilities.'),('World building: distinguish replay from prediction.','New actions test whether a reconstructed scene is useful.'),('Improvement: make real experiments repeatable.','Fixed interfaces, reset and verification support policy iteration.'),('Other applications: create engineering artifacts.','Designs and repaired programs need their own validation.')]
 for i,(title,detail) in enumerate(rows):
  y=170+i*107
...
order=[1,2,3,4,'exec','toolkits',5,6,7,8,9,'world',10,11,12,13,'improve',14,15,16,17,18,'other',19,20,22]
assert len(order)==26
chapter_keys={'exec','world','improve','other'}
minutes=[.5,2,2.5,1,2.5,2.5,3,3,2.5,2,1,2,2.5,3.5,3,2,2,3,4,3.5,3,2,1.5,2,2,1.5]
