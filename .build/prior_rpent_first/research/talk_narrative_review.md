...
审核对象：26 页 HTML 与同步中文讲稿。2026-09-10。
...
| Execution 5–11 | 哪些接口让 Agent 复用既有能力，各自代价是什么？ | RPent 原图与工具例子；Claude 原始 interface 图、direct/code control、familiar LIBERO-40、独立机器人评测与 wiping demo | 第 5 页多工具分支图导读；第 11 页讲稿收束，精细控制与试错成本引出 simulator |
...
- 第一章不再简化为 Agent → 单个 Controller。Action tools 分为 IK / motion planner、controller code、VLA / learned policy、WAM；perception/state tools 位于 observation 反馈路径。工具可选择、可组合，不是强制串行。
- 先解释接口优势，再看具体证据和限制；IK / WAM 是扩展例子，不伪装成 Claude Plays Robotics 的评测组。VLA proposal 先经 Agent accept/edit/replace，再到 robot。
- RPent 仅用一页原图与示意工具序列解释封装方式；ROSA / ROS-MCP 点到为止，不增加论文清单。
