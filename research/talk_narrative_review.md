...
审核对象：28 页 HTML 与同步中文讲稿。2026-09-11（0.5 版）。原 26 页审核记录保留在前，0.5 版补充见文末。
...
## 2026-09-11 — 三部分结构（0.5）

主线不变：Agent 调用什么工具，产生什么结果，用什么证据判断它有用。章节改按 Agent 的三种角色命名，每部分以流程图开场，结尾保留一页三角色之外的应用。

| 部分 | 提出的问题 | 对应证据 | 收束与过渡 |
|---|---|---|---|
| 开场 1–4 | Agent 与现有 robotics tools 如何合作？ | Painting demo 提供直觉，但不作为 weight update 证据 | 三个角色，明确不是强制流水线 |
| Agent Controls Robot 5–14 | 哪些接口让 Agent 复用既有能力，各自代价是什么？当前 demo 能说明什么，不能说明什么？ | Direct/code control → Claude model/interface 图 → familiar LIBERO-40 → Robocurve → 社区真机/仿真 demo → Show-Harness Table 2 → RPent 原图 → 缺口与方向 | 第 5 页导读；第 14 页把不足（接触精度、延迟、动力学、具身记忆）与方向（harness/工具库、扩展动作模型、仿真与数据、真实实验自改进）放在一页，过渡到第二、三部分 |
| Agent Produces Data 15–19 | 重建一段交互能支持哪些用途？ | 原流程图、DROID-100 全分母与 acceptance rule、独立失败 demo、小红书 Real2Sim 笔记 | 第 15 页流程图导读，产物定义为可运行的 simulated episode；第 19 页收束 replay 与 prediction 的区别，过渡到真实反馈 |
| Agent Post-trains Robot 20–26 | 真实实验能否支持 policy/skill improvement？ | Environment、reset/verifier、pin 曲线、token/time scaling、ASPIRE trace demo | 第 20 页固定 API 反馈图导读，说明 post-train 的对象包括 weights、训练程序和 skill 代码；第 26 页总结，保留 reset/retry 边界 |
| 结尾 27–28 | 三个角色之外还有什么？如何评价不同产物？ | Hand structural design | 第 27 页明确"Not a validated physical hand"；第 28 页回扣三个角色 |

### 本次证据口径

- 社区视频（第 10、11、14 页）均为作者自述 demo，页面标注作者、日期、倍速；讲稿明确无 trial 统计，不从画面推断可靠性或延迟。ARX 的"约四分钟"由 15.3 秒 × 16 倍速推出。
- Yanjie Ze 魔方：项目页说明为 MuJoCo physics replay、固定十步打乱；动作是否学习得到、是否实时、能否迁移到真机，帖子未说明，讲稿逐条列出。
- Show-Harness 数字只取 PDF Table 2（每任务 10 次 trial，作者自设任务集）；两次网页摘要给出的数字不一致，已弃用。
- 第 14 页的数字来源：Robocurve 19/20 与 2/20；HumanCLAW 单次 low-thinking 运行 46.6%；RoboDojo 一次社区运行 11/60；Malik、Max Fu、Yu Xiang 为观点引用，不作为实验结论。
- ASPIRE 归入第三部分的依据：它保留下来的产物是修好的 skill 代码，与 ENPIRE 改 policy/训练程序同属"rollout 之后仍然保留的改变"；讲稿说明这不等于 foundation model weights 更新。
- 小红书 "Robot RSI" 笔记只作为社区概括引用；其 RoboClaw 53.7% 数字未核实，不进入幻灯片。
