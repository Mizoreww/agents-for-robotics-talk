...
最终 HTML SHA-256：`a37c40df984ba507afcf6fce78443bc9c6c208c1ca9e02218bd7fb3ae0fb202c`。

## 实际范围

第6页 Hi Robot 结果替换为稀疏职责迁移图，末端按用户最新要求显示 System 0 层级及 Controller 功能名。保持第5页原架构、38页页序、18段媒体、其余数字与证据边界。Control 中文讲稿及过渡同步，英文字体与播放器样式不变。
...
0 项 findings。独立审查确认图以两条链和单一箭头表达 Partial shift，System 0 / Controller 命名已落实。讲稿明确 generalist 是目标、迁移是部分的、analytic tools 不等于 learned System 1、Astra 训练因果未披露。只有第6页画面变化。

## 命名补审

用户要求加回System 0。以刚完成的Controller-only版本为增量对照，两位reviewer再次通过，Standards 0项、Spec 0项。只恢复层级标签并同步讲稿，Controller与其余布局保持不变。对照存于`.build/v0_12_controller_only_snapshot/`，增量diff为`.build/v0_12_system0_followup.diff`。
